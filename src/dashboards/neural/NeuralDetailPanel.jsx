...
import { NEURAL_OVERLAY, NEURAL_LABELS, getNeuralStatusTier } from './neuralConfig.js';
...
const NeuralDetailPanel = ({ element, stats, onClose, getStatusTierFn }) => {
  const cat = element ? (activeCATRef.current[element.cat] ?? activeCATRef.current.TRANSITION) : null;
  const status = element ? getNeuralStatusTier(stats.level, getStatusTierFn) : null;
  const overlay = element ? NEURAL_OVERLAY[element.id] : null;
  const nodeClass = element ? (NEURAL_LABELS[element.cat] || element.cat) : null;
        ...
        >
          <div className="max-w-screen-2xl mx-auto px-6 py-4 h-full overflow-y-auto">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div style={{ fontSize: 11, color: cat.text, letterSpacing: '0.2em' }}>
                    {(overlay?.nodeName ?? element.name).toUpperCase()}
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.15em' }}>
                    {overlay?.nodeId ?? element.symbol} ◆ {overlay?.nodeClass ?? ''}
                  </div>
              ...
              <button
                onClick={onClose}
                aria-label="Close neural detail panel"
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <div style={{ ...SECTION_LABEL_STYLE, marginBottom: 6 }}>NODE_PARAMETERS</div>
                <div style={{ fontSize: 12, color: cat.text }}>{overlay?.signalStr ?? element.electronConfig}</div>
                <div style={{ ...SECTION_LABEL_STYLE, marginTop: 8, marginBottom: 4 }}>
                  TRANSMISSION_STATES ◆ [IDLE | TRANSMITTING | OVERLOADED | DARK_NODE]
                </div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)' }}>
                  {overlay?.packetRate ?? element.oxidation}
                </div>
...
export default NeuralDetailPanel;
