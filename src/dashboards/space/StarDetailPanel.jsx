...
import { SPACE_OVERLAY, STELLAR_LABELS, getSpaceStatusTier } from './spaceConfig.js';
...
const StarDetailPanel = ({ element, stats, onClose, getStatusTierFn }) => {
  const cat = element ? (activeCATRef.current[element.cat] ?? activeCATRef.current.TRANSITION) : null;
  const status = element ? getSpaceStatusTier(stats.level, getStatusTierFn) : null;
  const overlay = element ? SPACE_OVERLAY[element.id] : null;
  const stellarClass = element ? (STELLAR_LABELS[element.cat] || element.cat) : null;
          ...
          <div className="max-w-screen-2xl mx-auto px-6 py-4 h-full overflow-y-auto">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-4">
                {/* Big designation / stellar glow */}
                <div style={{ position: 'relative', width: 56, height: 56, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {/* Glow circle behind designation */}
                  <div style={{
                    position: 'absolute',
                    width: 48, height: 48,
                    borderRadius: '50%',
                  ...
                  <span style={{ fontSize: 22, fontWeight: 700, color: cat.text, textShadow: `0 0 20px ${cat.glow}`, lineHeight: 1, position: 'relative' }}>
                    {overlay?.designation ?? element.symbol}
                  </span>
                </div>
                <div>
                  <div style={{ fontSize: 11, color: cat.text, letterSpacing: '0.2em' }}>
                    {(overlay?.stellarName ?? element.name).toUpperCase()}
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.15em' }}>
                    {overlay?.spectralType ?? element.mass} ◆ {overlay?.solarMasses ?? ''}
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.3)', marginTop: 2, letterSpacing: '0.1em' }}>
                    {stellarClass?.toUpperCase()}
                  </div>
                </div>
              </div>
              <button
                onClick={onClose}
                aria-label="Close star detail panel"
            ...
            <div className="grid grid-cols-2 gap-6">
              {/* Left: Space telemetry */}
              <div className="space-y-2">
                <div style={{ ...SECTION_LABEL_STYLE, marginBottom: 6 }}>ORBITAL_PARAMETERS</div>
                <div style={{ fontSize: 12, color: cat.text }}>{overlay?.orbitLabel ?? element.electronConfig}</div>
                <div style={{ ...SECTION_LABEL_STYLE, marginTop: 8, marginBottom: 4 }}>
                  MISSION_STATES ◆ [MAIN_SEQUENCE | SOLAR_FLARE | RED_GIANT | SUPERNOVA]
                </div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)' }}>
                  {overlay?.flavor ?? element.oxidation}
                </div>
              ...
              {/* Right: Service telemetry (unchanged from chemistry mode) */}
              <div className="space-y-2">
...
export default StarDetailPanel;
