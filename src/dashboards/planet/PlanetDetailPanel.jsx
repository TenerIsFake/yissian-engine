...
import { PLANET_OVERLAY, PLANET_LABELS, getPlanetStatusTier } from './planetConfig.js';
...
const PlanetDetailPanel = ({ element, stats, onClose, getStatusTierFn }) => {
  const cat = element ? (activeCATRef.current[element.cat] ?? activeCATRef.current.TRANSITION) : null;
  const status = element ? getPlanetStatusTier(stats.level, getStatusTierFn) : null;
  const overlay = element ? PLANET_OVERLAY[element.id] : null;
  const nodeClass = element ? (PLANET_LABELS[element.cat] || element.cat) : null;
          ...
          <div className="max-w-screen-2xl mx-auto px-6 py-4 h-full overflow-y-auto">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-4">
                ...
                <div style={{ position: 'relative', width: 56, height: 56, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ position: 'absolute', width: 44, height: 14, border: `1px solid ${cat.border}`, borderRadius: '50%', opacity: stats.online ? 0.5 : 0.1, transform: 'rotate(-20deg)' }} />
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: stats.online ? `radial-gradient(circle at 35% 35%, ${cat.text}CC, ${cat.glow.replace(/[\d.]+\)$/, '0.35)')} 60%, transparent)` : 'rgba(255,255,255,0.08)', boxShadow: stats.online ? `0 0 16px ${cat.glow}` : 'none', position: 'relative', zIndex: 1 }} />
                </div>
                <div>
                  <div style={{ fontSize: 11, color: cat.text, letterSpacing: '0.2em' }}>
                    {(overlay?.planetName ?? element.name).toUpperCase()}
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.15em' }}>
                    {overlay?.orbitalClass ?? element.symbol} ◆ {overlay?.surfaceTemp ?? ''}
                  </div>
              ...
              <button onClick={onClose} aria-label="Close planet detail panel"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <div style={{ ...SECTION_LABEL_STYLE, marginBottom: 6 }}>ORBITAL_PARAMETERS</div>
                <div style={{ fontSize: 12, color: cat.text }}>{overlay?.atmosphere ?? element.electronConfig}</div>
                <div style={{ ...SECTION_LABEL_STYLE, marginTop: 8, marginBottom: 4 }}>
                  ORBITAL_STATES ◆ [ORBITING | ACTIVE | STORM | DARK_SIDE]
                </div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)' }}>{overlay?.surfaceTemp ?? element.oxidation}</div>
