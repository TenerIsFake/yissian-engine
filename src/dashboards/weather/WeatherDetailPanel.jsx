...
import { WEATHER_OVERLAY, WEATHER_LABELS, getWeatherStatusTier } from './weatherConfig.js';
...
const WeatherDetailPanel = ({ element, stats, onClose, getStatusTierFn }) => {
  const cat = element ? (activeCATRef.current[element.cat] ?? activeCATRef.current.TRANSITION) : null;
  const status = element ? getWeatherStatusTier(stats.level, getStatusTierFn) : null;
  const overlay = element ? WEATHER_OVERLAY[element.id] : null;
  const nodeClass = element ? (WEATHER_LABELS[element.cat] || element.cat) : null;
          ...
          <div className="max-w-screen-2xl mx-auto px-6 py-4 h-full overflow-y-auto">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-4">
                ...
                <div style={{ position: 'relative', width: 56, height: 56, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <span style={{ fontSize: 28, color: stats.online ? cat.text : 'rgba(255,255,255,0.12)', textShadow: stats.online ? `0 0 16px ${cat.glow}` : 'none', position: 'relative', zIndex: 1 }}>
                    {overlay?.weatherSymbol ?? '☁'}
                  </span>
                </div>
                <div>
                  <div style={{ fontSize: 11, color: cat.text, letterSpacing: '0.2em' }}>
                    {(overlay?.phenomena ?? element.name).toUpperCase()}
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.15em' }}>
                    {overlay?.stationId ?? element.symbol} ◆ {overlay?.barometric ?? ''}
                  </div>
              ...
              <button onClick={onClose} aria-label="Close weather detail panel"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <div style={{ ...SECTION_LABEL_STYLE, marginBottom: 6 }}>WEATHER_PARAMETERS</div>
                <div style={{ fontSize: 12, color: cat.text }}>{overlay?.weatherType ?? element.electronConfig}</div>
                <div style={{ ...SECTION_LABEL_STYLE, marginTop: 8, marginBottom: 4 }}>
                  FORECAST_STATES ◆ [CLEAR | ADVISORY | WARNING | BLACKOUT]
                </div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)' }}>{overlay?.windSpeed ? `Wind: ${overlay.windSpeed}` : element.oxidation}</div>
