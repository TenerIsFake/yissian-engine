...
import { GLOBE_OVERLAY, GLOBE_LABELS, getGlobeStatusTier } from './globeConfig.js';
...
// any simplifier suggestion to remove it. Pattern matches Band/ParticleDetailPanel.
const safeHref = (url) => {
  if (!url) return null;
  try {
...
const GlobeDetailPanel = ({ element, stats, onClose, getStatusTierFn }) => {
  const cat = element ? (activeCATRef.current[element.cat] ?? activeCATRef.current.TRANSITION) : null;
  const status = element ? getGlobeStatusTier(stats.level, getStatusTierFn) : null;
  const overlay = element ? GLOBE_OVERLAY[element.id] : null;
  const nodeClass = element ? (GLOBE_LABELS[element.cat] || element.cat) : null;
          ...
          <div className="max-w-screen-2xl mx-auto px-6 py-4 h-full overflow-y-auto">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-4">
                ...
                <div style={{ position: 'relative', width: 56, height: 56, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ position: 'absolute', width: 46, height: 46, borderRadius: '50%', border: `1px solid ${cat.border}`, opacity: stats.online ? 0.7 : 0.1 }} />
                  <div style={{ position: 'absolute', width: 46, height: 0, borderTop: `1px solid ${cat.border}`, opacity: stats.online ? 0.4 : 0.07 }} />
                  <div style={{ position: 'absolute', width: 34, height: 0, borderTop: `1px solid ${cat.border}`, opacity: stats.online ? 0.25 : 0.05, transform: 'translateY(-12px)' }} />
                  <div style={{ position: 'absolute', width: 34, height: 0, borderTop: `1px solid ${cat.border}`, opacity: stats.online ? 0.25 : 0.05, transform: 'translateY(12px)' }} />
                  <div style={{ position: 'absolute', width: 0, height: 46, borderLeft: `1px solid ${cat.border}`, opacity: stats.online ? 0.35 : 0.07 }} />
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: stats.online ? cat.text : 'rgba(255,255,255,0.1)', boxShadow: stats.online ? `0 0 10px ${cat.glow}` : 'none', position: 'relative', zIndex: 1 }} />
                </div>
                <div>
                  <div style={{ fontSize: 11, color: cat.text, letterSpacing: '0.2em' }}>
                    {(overlay?.region ?? element.name).toUpperCase()}
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.15em' }}>
                    {overlay?.coordinates ?? element.symbol} ◆ {overlay?.climate ?? ''}
                  </div>
              ...
              <button onClick={onClose} aria-label="Close globe detail panel"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <div style={{ ...SECTION_LABEL_STYLE, marginBottom: 6 }}>GEO_PARAMETERS</div>
                <div style={{ fontSize: 12, color: cat.text }}>{overlay?.elevation ?? element.electronConfig}</div>
                <div style={{ ...SECTION_LABEL_STYLE, marginTop: 8, marginBottom: 4 }}>
                  TERRAIN_STATES ◆ [STABLE | MONITORING | ALERT | OFFLINE]
                </div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)' }}>
                  {overlay ? `Climate: ${overlay.climate}` : element.oxidation}
                </div>
...
export default GlobeDetailPanel;
