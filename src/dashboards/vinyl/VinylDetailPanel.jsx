...
import { VINYL_OVERLAY, VINYL_LABELS, getVinylStatusTier } from './vinylConfig.js';
...
const VinylDetailPanel = ({ element, stats, onClose, getStatusTierFn }) => {
  const cat = element ? (activeCATRef.current[element.cat] ?? activeCATRef.current.TRANSITION) : null;
  const status = element ? getVinylStatusTier(stats.level, getStatusTierFn) : null;
  const overlay = element ? VINYL_OVERLAY[element.id] : null;
  const nodeClass = element ? (VINYL_LABELS[element.cat] || element.cat) : null;
          ...
          <div className="max-w-screen-2xl mx-auto px-6 py-4 h-full overflow-y-auto">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-4">
                ...
                <div style={{ position: 'relative', width: 56, height: 56, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ position: 'absolute', width: 52, height: 52, borderRadius: '50%', border: `1px solid ${cat.border}`, opacity: stats.online ? 0.5 : 0.08 }} />
                  <div style={{ position: 'absolute', width: 36, height: 36, borderRadius: '50%', border: `1px solid ${cat.border}`, opacity: stats.online ? 0.35 : 0.06 }} />
                  <div style={{ position: 'absolute', width: 20, height: 20, borderRadius: '50%', border: `1px solid ${cat.border}`, opacity: stats.online ? 0.5 : 0.08 }} />
                ...
                <div>
                  <div style={{ fontSize: 11, color: cat.text, letterSpacing: '0.2em' }}>
                    {(overlay?.artist ?? element.name).toUpperCase()}
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.15em' }}>
                    {overlay?.catalogNum ?? element.symbol} ◆ {overlay?.rpm ?? ''} RPM
                  </div>
              ...
              <button onClick={onClose} aria-label="Close vinyl detail panel"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <div style={{ ...SECTION_LABEL_STYLE, marginBottom: 6 }}>RECORD_PARAMETERS</div>
                <div style={{ fontSize: 12, color: cat.text }}>{overlay?.genre ?? element.electronConfig}</div>
                <div style={{ ...SECTION_LABEL_STYLE, marginTop: 8, marginBottom: 4 }}>
                  PLAYBACK_STATES ◆ [SPINNING | CUED | WARPED | SCRATCHED]
                </div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)' }}>{overlay?.label ? `Label: ${overlay.label}` : element.oxidation}</div>
