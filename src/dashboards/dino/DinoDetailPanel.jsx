...
import { DINO_OVERLAY, DINO_LABELS, getDinoStatusTier } from './dinoConfig.js';
...
const DinoDetailPanel = ({ element, stats, onClose, getStatusTierFn }) => {
  const cat = element ? (activeCATRef.current[element.cat] ?? activeCATRef.current.TRANSITION) : null;
  const status = element ? getDinoStatusTier(stats.level, getStatusTierFn) : null;
  const overlay = element ? DINO_OVERLAY[element.id] : null;
  const nodeClass = element ? (DINO_LABELS[element.cat] || element.cat) : null;
          ...
          <div className="max-w-screen-2xl mx-auto px-6 py-4 h-full overflow-y-auto">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-4">
                {/* Claw marks visual */}
                <div style={{ position: 'relative', width: 56, height: 56, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
                  <div style={{ position: 'absolute', width: 48, height: 48, borderRadius: '50%', background: stats.online ? `radial-gradient(circle at 40% 40%, ${cat.text}22, transparent)` : 'transparent', boxShadow: stats.online ? `0 0 20px ${cat.glow}` : 'none' }} />
                  {[-1, 0, 1].map(i => (
                    <div key={i} style={{ width: 3, height: 28, background: stats.online ? `linear-gradient(to bottom, ${cat.text}, transparent)` : 'rgba(255,255,255,0.08)', borderRadius: 1, transform: `rotate(${i * 14}deg)`, boxShadow: stats.online ? `0 0 6px ${cat.glow}` : 'none', position: 'relative', zIndex: 1 }} />
                  ))}
                </div>
                <div>
                  <div style={{ fontSize: 11, color: cat.text, letterSpacing: '0.2em' }}>
                    {(overlay?.species ?? element.name).toUpperCase()}
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.15em' }}>
                    {overlay?.clade ?? element.symbol} ◆ {overlay?.epoch ?? ''}
                  </div>
              ...
              <button onClick={onClose} aria-label="Close dino detail panel"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <div style={{ ...SECTION_LABEL_STYLE, marginBottom: 6 }}>SPECIMEN_PARAMETERS</div>
                <div style={{ fontSize: 12, color: cat.text }}>{overlay?.clade ?? element.electronConfig}</div>
                <div style={{ ...SECTION_LABEL_STYLE, marginTop: 8, marginBottom: 4 }}>
                  ACTIVITY_STATES ◆ [ACTIVE | DORMANT | MIGRATING | EXTINCT]
                </div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)' }}>{overlay?.region ?? element.oxidation}</div>
...
export default DinoDetailPanel;
