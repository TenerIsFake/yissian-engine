...
import { BAND_OVERLAY, BAND_LABELS, getBandStatusTier } from './bandConfig.js';
...
// element.url is compile-time config today; this guard prevents javascript: URI injection
// if a future config or data migration introduces user-supplied URL values.
// SECURITY OVERRIDE: safeHref utility retained even though pentester rates this Low/Info
// (compile-time safe currently) — overrides any simplifier suggestion to remove it.
const safeHref = (url) => {
  if (!url) return null;
  try {
...
const BandDetailPanel = ({ element, stats, onClose, getStatusTierFn }) => {
  const cat = element ? (activeCATRef.current[element.cat] ?? activeCATRef.current.TRANSITION) : null;
  const status = element ? getBandStatusTier(stats.level, getStatusTierFn) : null;
  const overlay = element ? BAND_OVERLAY[element.id] : null;
  const nodeClass = element ? (BAND_LABELS[element.cat] || element.cat) : null;
          ...
          <div className="max-w-screen-2xl mx-auto px-6 py-4 h-full overflow-y-auto">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-4">
                {/* Equalizer visual */}
                <div style={{ position: 'relative', width: 56, height: 56, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', gap: 4, paddingBottom: 6 }}>
                  {[18, 28, 22, 32, 16].map((h, i) => (
                    <div key={i} style={{
                      width: 5, height: h,
                      background: stats.online ? `linear-gradient(to top, ${cat.border}, ${cat.text})` : 'rgba(255,255,255,0.08)',
                      borderRadius: 2,
                ...
                <div>
                  <div style={{ fontSize: 11, color: cat.text, letterSpacing: '0.2em' }}>
                    {(overlay?.bandName ?? element.name).toUpperCase()}
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.15em' }}>
                    {overlay?.role ?? element.symbol} ◆ {overlay?.era ?? ''}
                  </div>
              ...
              <button onClick={onClose} aria-label="Close band detail panel"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <div style={{ ...SECTION_LABEL_STYLE, marginBottom: 6 }}>BAND_PARAMETERS</div>
                <div style={{ fontSize: 12, color: cat.text }}>{overlay?.instrument ?? element.electronConfig}</div>
                <div style={{ ...SECTION_LABEL_STYLE, marginTop: 8, marginBottom: 4 }}>
                  PERFORMANCE_STATES ◆ [TOURING | REHEARSING | RECORDING | DISBANDED]
                </div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)' }}>{overlay?.role ?? element.oxidation}</div>
