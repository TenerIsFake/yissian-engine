...
import { MOLECULE_OVERLAY, MOLECULE_LABELS, getMoleculeStatusTier } from './moleculeConfig.js';
...
const MoleculeDetailPanel = ({ element, stats, onClose, getStatusTierFn }) => {
  const cat = element ? (activeCATRef.current[element.cat] ?? activeCATRef.current.TRANSITION) : null;
  const status = element ? getMoleculeStatusTier(stats.level, getStatusTierFn) : null;
  const overlay = element ? MOLECULE_OVERLAY[element.id] : null;
  const nodeClass = element ? (MOLECULE_LABELS[element.cat] || element.cat) : null;
          ...
          <div className="max-w-screen-2xl mx-auto px-6 py-4 h-full overflow-y-auto">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-4">
                {/* Molecule atom visual */}
                <div style={{ position: 'relative', width: 56, height: 56, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ position: 'absolute', width: 48, height: 48, borderRadius: '50%', background: stats.online ? `radial-gradient(circle at 40% 40%, ${cat.text}33, ${cat.glow.replace(/[\d.]+\)$/, '0.15)')} 60%, transparent)` : 'transparent', boxShadow: stats.online ? `0 0 20px ${cat.glow}` : 'none' }} />
                  <div style={{ fontSize: 22, color: stats.online ? cat.text : 'rgba(255,255,255,0.12)', textShadow: stats.online ? `0 0 12px ${cat.glow}` : 'none', fontFamily: MONO, position: 'relative', zIndex: 1 }}>
                    {overlay?.formula ?? element.symbol}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: 11, color: cat.text, letterSpacing: '0.2em' }}>
                    {(overlay?.compoundName ?? element.name).toUpperCase()}
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.15em' }}>
                    {overlay?.compoundType ?? element.symbol} ◆ {overlay?.molarMass ?? ''}
                  </div>
              ...
              <button onClick={onClose} aria-label="Close molecule detail panel"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <div style={{ ...SECTION_LABEL_STYLE, marginBottom: 6 }}>MOLECULAR_PARAMETERS</div>
                <div style={{ fontSize: 12, color: cat.text }}>{overlay?.bondType ?? element.electronConfig}</div>
                <div style={{ ...SECTION_LABEL_STYLE, marginTop: 8, marginBottom: 4 }}>
                  BOND_STATES ◆ [BONDED | REACTING | UNSTABLE | DISSOCIATED]
                </div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)' }}>{overlay?.molarMass ?? element.oxidation}</div>
