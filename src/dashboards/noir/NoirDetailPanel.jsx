...
import { NOIR_OVERLAY, NOIR_LABELS, getNoirStatusTier } from './noirConfig.js';
...
const NoirDetailPanel = ({ element, stats, onClose, getStatusTierFn }) => {
  const cat = element ? (activeCATRef.current[element.cat] ?? activeCATRef.current.TRANSITION) : null;
  const status = element ? getNoirStatusTier(stats.level, getStatusTierFn) : null;
  const overlay = element ? NOIR_OVERLAY[element.id] : null;
  const nodeClass = element ? (NOIR_LABELS[element.cat] || element.cat) : null;
          ...
          <div className="max-w-screen-2xl mx-auto px-6 py-4 h-full overflow-y-auto">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-4">
                ...
                <div style={{ position: 'relative', width: 56, height: 56, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ position: 'absolute', width: 36, height: 36, border: `1px solid ${cat.border}`, transform: 'rotate(45deg)', opacity: stats.online ? 0.6 : 0.1 }} />
                  <div style={{ position: 'absolute', width: 22, height: 22, border: `1px solid ${cat.border}`, transform: 'rotate(45deg)', opacity: stats.online ? 0.3 : 0.05 }} />
                ...
                <div>
                  <div style={{ fontSize: 11, color: cat.text, letterSpacing: '0.2em' }}>
                    {(overlay?.alias ?? element.name).toUpperCase()}
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.15em' }}>
                    {overlay?.caseNum ?? element.symbol} ◆ {overlay?.rank ?? ''}
                  </div>
              ...
              <button onClick={onClose} aria-label="Close noir detail panel"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <div style={{ ...SECTION_LABEL_STYLE, marginBottom: 6 }}>CASE_PARAMETERS</div>
                <div style={{ fontSize: 12, color: cat.text }}>{overlay?.bureau ?? element.electronConfig}</div>
                <div style={{ ...SECTION_LABEL_STYLE, marginTop: 8, marginBottom: 4 }}>
                  OPERATIVE_STATES ◆ [ACTIVE | TAILING | COMPROMISED | GONE_DARK]
                </div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)' }}>{overlay?.specialty ?? element.oxidation}</div>
...
export default NoirDetailPanel;
