...
import { PARTICLE_OVERLAY, PARTICLE_LABELS, getParticleStatusTier } from './particleConfig.js';
...
// SEC-04 / L-04: protocol allowlist guard — defense-in-depth for element.url.
// SECURITY OVERRIDE: retained even though currently compile-time safe — overrides
// any simplifier suggestion to remove it. Pattern matches Band/GlobeDetailPanel.
const safeHref = (url) => {
  if (!url) return null;
  try {
...
const ParticleDetailPanel = ({ element, stats, onClose, getStatusTierFn }) => {
  const cat = element ? (activeCATRef.current[element.cat] ?? activeCATRef.current.TRANSITION) : null;
  const status = element ? getParticleStatusTier(stats.level, getStatusTierFn) : null;
  const overlay = element ? PARTICLE_OVERLAY[element.id] : null;
  const nodeClass = element ? (PARTICLE_LABELS[element.cat] || element.cat) : null;
          ...
          <div className="max-w-screen-2xl mx-auto px-6 py-4 h-full overflow-y-auto">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-4">
                ...
                <div style={{ position: 'relative', width: 56, height: 56, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ position: 'absolute', width: 48, height: 28, borderRadius: '50%', border: `1px dashed ${cat.border}`, opacity: stats.online ? 0.6 : 0.1 }} />
                  <div style={{ position: 'absolute', width: 48, height: 28, borderRadius: '50%', border: `1px dashed ${cat.border}`, opacity: stats.online ? 0.4 : 0.07, transform: 'rotate(60deg)' }} />
                  <div style={{ position: 'absolute', width: 48, height: 28, borderRadius: '50%', border: `1px dashed ${cat.border}`, opacity: stats.online ? 0.3 : 0.05, transform: 'rotate(-60deg)' }} />
                  <div style={{ width: 10, height: 10, borderRadius: '50%', background: stats.online ? cat.text : 'rgba(255,255,255,0.1)', boxShadow: stats.online ? `0 0 12px ${cat.glow}` : 'none', position: 'relative', zIndex: 1 }} />
                </div>
                <div>
                  <div style={{ fontSize: 11, color: cat.text, letterSpacing: '0.2em' }}>
                    {(overlay?.particleName ?? element.name).toUpperCase()}
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.15em' }}>
                    {overlay?.symbol ?? element.symbol} ◆ CHARGE: {overlay?.charge ?? ''}
                  </div>
              ...
              <button onClick={onClose} aria-label="Close particle detail panel"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <div style={{ ...SECTION_LABEL_STYLE, marginBottom: 6 }}>PARTICLE_PARAMETERS</div>
                <div style={{ fontSize: 12, color: cat.text }}>{overlay?.mass ?? element.electronConfig}</div>
                <div style={{ ...SECTION_LABEL_STYLE, marginTop: 8, marginBottom: 4 }}>
                  QUANTUM_STATES ◆ [STABLE | EXCITED | DECAYING | ANNIHILATED]
                </div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)' }}>
                  {overlay ? `Spin: ${overlay.spin} ◆ Charge: ${overlay.charge}` : element.oxidation}
                </div>
...
export default ParticleDetailPanel;
