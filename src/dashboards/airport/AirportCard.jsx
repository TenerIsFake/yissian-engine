...
const AirportCard = ({ element, stats, onClick }) => {
  const cat = activeCATRef.current[element.cat] ?? activeCATRef.current.TRANSITION;
  const isOffline = !stats.online;
  const overlay = AIRPORT_OVERLAY[element.id];

  const icao        = overlay?.icao        ?? element.symbol;
  const flightDesig = overlay?.flightDesig ?? element.service ?? element.name;
