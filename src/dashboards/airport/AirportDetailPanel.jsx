...
import { AIRPORT_OVERLAY, AIRPORT_LABELS, getAirportStatusTier } from './airportConfig.js';
...
// SECURITY: Blocks javascript: URIs and non-http(s) schemes from element.url
const safeHref = (url) => {
  if (!url) return null;
  try {
...
const AirportDetailPanel = ({ element, stats, onClose, getStatusTierFn }) => {
  const cat = element ? (activeCATRef.current[element.cat] ?? activeCATRef.current.TRANSITION) : null;
  const status = element ? getAirportStatusTier(stats.level, getStatusTierFn) : null;
  const overlay = element ? AIRPORT_OVERLAY[element.id] : null;
  const nodeClass = element ? (AIRPORT_LABELS[element.cat] || element.cat) : null;
          ...
          <div className="max-w-screen-2xl mx-auto px-6 py-4 h-full overflow-y-auto">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-4">
                ...
                <div style={{ position: 'relative', width: 56, height: 56, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <span style={{ fontSize: 28, color: stats.online ? cat.text : 'rgba(255,255,255,0.12)', textShadow: stats.online ? `0 0 16px ${cat.glow}` : 'none', transform: 'rotate(-45deg)', display: 'inline-block', position: 'relative', zIndex: 1 }}>✈</span>
                </div>
                <div>
                  <div style={{ fontSize: 11, color: cat.text, letterSpacing: '0.2em' }}>
                    {(overlay?.flightDesig ?? element.name).toUpperCase()}
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.15em' }}>
                    {overlay?.icao ?? element.symbol} ◆ {overlay?.gateInfo ?? ''}
                  </div>
              ...
              <button onClick={onClose} aria-label="Close airport detail panel"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <div style={{ ...SECTION_LABEL_STYLE, marginBottom: 6 }}>FLIGHT_PARAMETERS</div>
                <div style={{ fontSize: 12, color: cat.text }}>{overlay?.terminal ?? element.electronConfig}</div>
                <div style={{ ...SECTION_LABEL_STYLE, marginTop: 8, marginBottom: 4 }}>
                  FLIGHT_STATES ◆ [TAXIING | AIRBORNE | HOLDING | GROUNDED]
                </div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)' }}>{overlay?.runway ?? element.oxidation}</div>
