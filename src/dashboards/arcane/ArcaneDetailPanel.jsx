...
import { ARCANE_OVERLAY, ARCANE_LABELS, getArcaneStatusTier } from './arcaneConfig.js';
...
const ArcaneDetailPanel = ({ element, stats, onClose, getStatusTierFn }) => {
  const cat = element ? (activeCATRef.current[element.cat] ?? activeCATRef.current.TRANSITION) : null;
  const status = element ? getArcaneStatusTier(stats.level, getStatusTierFn) : null;
  const overlay = element ? ARCANE_OVERLAY[element.id] : null;
  const tomeClass = element ? (ARCANE_LABELS[element.cat] || element.cat) : null;
          ...
          <div className="max-w-screen-2xl mx-auto px-6 py-4 h-full overflow-y-auto">
              ...
              <div className="flex items-center gap-4">
                ...
                <div style={{ position: 'relative', width: 56, height: 56, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <span style={{
                    ...
                    textShadow: stats.online ? `0 0 20px ${cat.glow}` : 'none',
                    lineHeight: 1,
                    position: 'relative',
                  }}>
                    {overlay?.runeSymbol ?? element.symbol}
                  </span>
                </div>
                <div>
                  <div style={{ fontSize: 11, color: cat.text, letterSpacing: '0.2em' }}>
                    {(overlay?.tomeTitle ?? element.name).toUpperCase()}
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.15em' }}>
                    {overlay?.tomeType ?? ''} ◆ {overlay?.accessTier ?? ''}
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.3)', marginTop: 2, letterSpacing: '0.1em' }}>
                    {tomeClass?.toUpperCase()}
                  </div>
                </div>
              </div>
              <button
                onClick={onClose}
                aria-label="Close arcane detail panel"
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <div style={{ ...SECTION_LABEL_STYLE, marginBottom: 6 }}>TOME_PARAMETERS</div>
                <div style={{ fontSize: 12, color: cat.text }}>{overlay?.bindingPower ?? element.electronConfig}</div>
                <div style={{ ...SECTION_LABEL_STYLE, marginTop: 8, marginBottom: 4 }}>
                  ARCANE_STATES ◆ [AWAKENED | DORMANT | CORRUPTED | SEALED]
                </div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)' }}>
                  {overlay?.accessTier ?? element.oxidation}
                </div>
...
export default ArcaneDetailPanel;
